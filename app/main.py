...
def get_stations():
    ...
    stations = []
    #conn=engine.connect()
    rows = engine.execute("SELECT * from StaticData order by number;")
    for row in rows:
...
def getWeather():
    ...
    weather = []
    #conn = engine.connect()
    rows = engine.execute("SELECT * FROM DynamicData.weatherData order by DATE desc, time desc limit 1 ;")
    for row in rows:
